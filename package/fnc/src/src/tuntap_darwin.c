...
int
fnc_tuntap_read(struct tuntap_s *tuntap)
{
    ssize_t ret;
    ...
    return fnc_tuntap_generic_read(data, ret);
}

int
fnc_tuntap_write(struct tuntap_s *tuntap)
{
    ssize_t ret;
    fnc_pkt_t *pkt;
    ...
    /* Safety checks */
    if (fnc_cb_is_empty(buf))
        fatalx("tuntap_write called with empty buffer");

    pkt = fnc_pktbuffer_read(buf);
...
int
fnc_tuntap_alloc(struct tuntap_s *tuntap)
{
    char devname[8];
    int fd;
    int i;
    ...
    for (i=0; i < 32; i++)
    {
        snprintf(devname, 5, "%s%d",
                 tuntap->type == FNC_TUNTAPMODE_TAP ? "tap" : "tun", i);
    ...
    if (fd <= 0)
    {
        log_warnx("tuntap",
            ...
            tuntap->type == FNC_TUNTAPMODE_TAP ? "tap" : "tun");
        return fd;
    }
    tuntap->fd = fd;
