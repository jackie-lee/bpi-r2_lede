...
/*
 * fnc can only go forward in these states; each state should represent
...
priv_init(char *argv[], char *username)
{
    ...
    char hostname[FNC_MAXHNAMSTR], servname[FNC_MAXHNAMSTR];
    char *phostname, *pservname;
    char script_path[MAXPATHLEN] = {0};
    char tuntapname[FNC_IFNAMSIZ];
    char **script_argv;
    char **script_env;
...
/* NetBSD does not have thoses */
    ...
    while (cur_state < STATE_QUIT)
    {
        ...
        case PRIV_OPEN_TUN:
            ...
            if (tuntapmode != FNC_TUNTAPMODE_TUN &&
                    tuntapmode != FNC_TUNTAPMODE_TAP)
                _exit(0);
...
/* Open fnc config file for reading */
int
...
int priv_open_tun(int tuntapmode, char *devname, int mtu)
{
    ...
    if (len > 0 && len < FNC_IFNAMSIZ && devname != NULL)
    {
...
{
    char hostcpy[FNC_MAXHNAMSTR], servcpy[FNC_MAXHNAMSTR];
    int cmd;
...
sig_got_chld(int sig)
{
    ...
    do {
        ...
        pid = waitpid(-1, NULL, WNOHANG); // ljk, for musl
