...
dnl fnc_AC_EXPAND(var)

AC_DEFUN([fnc_AC_EXPAND], [
...
dnl fnc_ARG_WITH(name, help1, default)

AC_DEFUN([fnc_ARG_WITH],[
  AC_ARG_WITH([$1],
    AS_HELP_STRING([--with-$1],
        [$2 @<:@default=$3@:>@]),[
        fnc_AC_EXPAND("$withval", expanded)
        AC_DEFINE_UNQUOTED(AS_TR_CPP([$1]), ["$expanded"], [$2])
        AC_SUBST(AS_TR_CPP([$1]), [$expanded])],[
        fnc_AC_EXPAND("$3", expanded)
    AC_DEFINE_UNQUOTED(AS_TR_CPP([$1]), ["$expanded"], [$2])
        AC_SUBST(AS_TR_CPP([$1]), [$expanded])
        eval with_[]m4_translit([$1], [-+.], [___])="$expanded"
])])

dnl fnc_ARG_ENABLE(name, help1, default)

AC_DEFUN([fnc_ARG_ENABLE],[
  AC_ARG_ENABLE([$1],
